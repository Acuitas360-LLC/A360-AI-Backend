...
class DeckContentGenerationError(RuntimeError):
    """Raised when PPT slide content cannot be generated by any configured LLM."""


def _model_for_provider(cfg: DeckConfig, provider: str) -> str:
    if provider == "anthropic":
        return cfg.llm_model or _env_nonempty("ANTHROPIC_MODEL") or "claude-sonnet-5"
    if provider == "openai":
        return cfg.llm_model or _env_nonempty("OPENAI_MODEL") or "gpt-4.1"
    return cfg.llm_model or "(default)"


def _provider_sequence(cfg: DeckConfig) -> list[str]:
    provider = _resolve_provider(cfg)
    has_openai = bool(_env_nonempty("OPENAI_API_KEY") or _env_nonempty("AZURE_OPENAI_API_KEY"))
    if provider == "anthropic":
        sequence = ["anthropic"]
        if has_openai:
...
def _call_llm_with_provider(system: str, messages: list[dict], cfg: DeckConfig,
                            provider: str) -> str:
    model = _model_for_provider(cfg, provider)
    if provider == "anthropic":
    ...
    raise DeckContentGenerationError(
        f"Unsupported PPT content provider '{provider}'."
    )


def _call_llm(system: str, messages: list[dict], cfg: DeckConfig) -> str:
    provider = _resolve_provider(cfg)
    providers = _provider_sequence(cfg)
    ...
    if not providers:
        raise DeckContentGenerationError(
            "No LLM provider configured for PPT content generation."
        )

    failures: list[str] = []
    for index, candidate in enumerate(providers):
        model = _model_for_provider(cfg, candidate)
        try:
            return _call_llm_with_provider(system, messages, cfg, candidate)
        except Exception as exc:
            failures.append(f"{candidate}/{model}: {exc}")
            if index + 1 < len(providers):
                log.warning("LLM call failed (%s/%s): %s; trying %s",
                            candidate, model, exc, providers[index + 1])
            else:
                log.warning("LLM call failed (%s/%s): %s",
                            candidate, model, exc)

    raise DeckContentGenerationError(
        "PPT content generation failed across providers: "
        + " | ".join(failures)
    )
...
def enforce_content_rules(content: dict, df, profile: dict,
    ...
    points = enforce_variety(content.get("bullets") or [], lambda x: x,
                             min_items=0, max_items=cfg.max_bullets)
    content["bullets"] = points[:cfg.max_bullets]
    return content
...
def generate_slide_content(block: dict, cfg: DeckConfig | None = None,
    ...
    """
    LLM content with strict validation and repair retries.
    """
    ...
    for attempt in range(cfg.llm_max_retries + 1):
        ...
        if raw is None:
            ...
            if not problems:
                ...
                return _finalise_content(content, block, df, profile, cfg)
    ...
    raise DeckContentGenerationError(
        "PPT content generation returned unresolved content errors for "
        f"'{_debug_excerpt(block.get('question'), 80)}': {problems}"
    )


def _finalise_content(content: dict, block: dict, df, profile: dict,
...
def build_deck(messages: Iterable[Any], cfg: DeckConfig | None = None) -> str:
    ...
    Fail fast: content generation errors should surface to the caller.
    """
    cfg = cfg or DeckConfig()
...
def _finalise_chart_content(content: dict, df, profile: dict, cfg: DeckConfig | None = None, chart: dict | None = None) -> dict:
    forced_note = _incomplete_period_note(df, profile)
    if forced_note and forced_note.lower() not in (content.get("footnote") or "").lower():
        content["footnote"] = forced_note
...
def generate_slide_content_with_chart(block: dict, cfg: DeckConfig, df, profile: dict, chart: dict | None = None) -> dict:
    ...
    for attempt in range(cfg.llm_max_retries + 1):
        ...
        if raw is None:
            ...
            if not problems:
                return _finalise_chart_content(content, df, profile, cfg, chart)
        if attempt < cfg.llm_max_retries:
            messages = messages + [
                {"role": "assistant", "content": reply},
